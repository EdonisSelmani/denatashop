...
<div class="space-y-7">
    <div class="flex items-center justify-between">
        ...
        <a href="{{ route('shop') }}" class="text-sm font-bold text-[#9A712E] hover:text-[#15181B]">Pastro</a>
    </div>
    ...
    <div>
        ...
        <div class="mt-3 space-y-2">
            @foreach($categories as $category)
                ...
                <div class="rounded-md border {{ $isCategoryActive ? 'border-[#B88A3B] bg-[#B88A3B]/10' : 'border-[#E5E1DA] bg-white' }}">
                    <a href="{{ route('shop', array_merge(request()->except('category', 'subcategory', 'page'), ['category' => $category->slug])) }}"
                       class="flex items-center justify-between px-3 py-2 text-sm font-bold {{ $isCategoryActive ? 'text-[#9A712E]' : 'text-[#22272B]' }}">
                        {{ $category->name }}
                        <span class="text-xs text-[#6B6F74]">{{ $category->products_count ?? $category->active_products_count ?? '' }}</span>
                    </a>
                            ...
                            <div class="grid gap-1">
                                @foreach($category->subcategories as $subcategory)
                                    <a href="{{ route('shop', array_merge(request()->except('category', 'subcategory', 'page'), ['category' => $category->slug, 'subcategory' => $subcategory->slug])) }}"
                                       class="rounded px-2 py-1.5 text-sm {{ request('subcategory') === $subcategory->slug ? 'bg-white font-bold text-[#9A712E]' : 'text-[#6B6F74] hover:bg-[#F7F5F1] hover:text-[#9A712E]' }}">
                                        {{ $subcategory->name }}
                                    </a>
                                @endforeach
                            </div>
    ...
    <form action="{{ route('shop') }}" method="GET" class="space-y-6">
        ...
        <div>
            ...
            <label class="mt-3 flex items-center gap-3 rounded-md border border-[#E5E1DA] bg-white px-3 py-3 text-sm font-bold text-[#22272B]">
